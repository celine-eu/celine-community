...
async def test_cache_reuses_value_for_the_same_rec_and_period() -> None:
    ...
    first, first_hit = await cache.get_or_set(("overview", "greenland", "7d"), produce)
    second, second_hit = await cache.get_or_set(("overview", "greenland", "7d"), produce)
    other_period, _ = await cache.get_or_set(("overview", "greenland", "30d"), produce)
...
async def test_cache_prevents_a_same_key_stampede() -> None:
    ...
    values = await asyncio.gather(
        *(cache.get_or_set(("chain", "greenland", "30d"), produce) for _ in range(5))
    )
